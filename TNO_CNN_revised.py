...
def convnet_model(input_shape, training_labels, unique_labs, dropout_rate=dropout_rate):
    ...
    #hidden layer 1
    model.add(Conv3D(filters=8*4, kernel_size=(1, 3, 3), input_shape=input_shape, activation='relu', padding='valid'))
    ...
    model.add(Flatten())
    model.add(Dense(16*4, activation='sigmoid')) # 128 -> 64
...
ax1.plot(classifier.history['val_accuracy'], linewidth=2, label='testing')
ax1.legend()
